...
#ifndef MATRIXACCUMULATORS_H_
#define MATRIXACCUMULATORS_H_
...
public:
  ...
  {
    A = Eigen::Matrix<float,i,j>::Zero();
    A1k = Eigen::Matrix<float,i,j>::Zero();
    A1m = Eigen::Matrix<float,i,j>::Zero();
...
public:
  ...
  inline void initialize()
  {
    A = Eigen::Matrix<float,i,1>::Zero();
    A1k = Eigen::Matrix<float,i,1>::Zero();
    A1m = Eigen::Matrix<float,i,1>::Zero();
    num = numIn1 = numIn1k = numIn1m = 0;
  }
...
public:
  ...
  inline void updateSSE(AccumulatorApprox
                  ...
                  const __m128 J12,const __m128 J13)
  {
      ...
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J1,J9))); AccumulatorApproxpt+=4;
                  ...
                  int off=0)
  {
      ...
      *pt += J9*J1; pt+=4;
      *pt += J10*J1; pt+=4;
      *pt += J11*J1; pt+=4;AccumulatorApprox
      *pt += J12*J1; pt+=4;
      *pt += J13*J1; pt+=4;
...
inline void finish()
{
    H = Eigen::Matrix<float,13,13>::Zero();
    shiftUp(true);
    assert(numIn1==0);
    assert(numIn1k==0);
...
public:
  ...
  inline void initialize()
  {
      H = Eigen::Matrix<float,7,7>::Zero();
      b = Eigen::Matrix<float,7,1>::Zero();
  ...
  inline void updateSSE_weighted(
                  ...
                  const __m128 J2,const __m128 J3,
                  const __m128 J4,const __m128 J5,
                  const __m128 J6, const __m128 w)
  {
      float* pt=SSEData;
...
/*
 * computes the outer sum of 7x2 matrices, weighted with a 2x2 matrix:
 * 			H = [x y] * [a b; b c] * [x y]^T
 * (assuming x,y are column-vectors).
 * numerically robust to large sums.
 */
class Accumulator7_2
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  inline void updateSSE(
                  const __m128 J00,const __m128 J01,const __m128 J02,const __m128 J03, const __m128 J04,const __m128 J05,const __m128 J06,
                  const __m128 J10,const __m128 J11,const __m128 J12,const __m128 J13, const __m128 J14,const __m128 J15,const __m128 J16,
                  const float a, const float b, const float c)
  {
      __m128 w00 = _mm_set1_ps(a);
      __m128 w01 = _mm_set1_ps(b);
      __m128 w11 = _mm_set1_ps(c);

      float* pt=SSEData;
      __m128 tmp1 = _mm_add_ps(_mm_mul_ps(J00, w00), _mm_mul_ps(J10, w01));
      __m128 tmp2 = _mm_add_ps(_mm_mul_ps(J00, w01), _mm_mul_ps(J10, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J00), _mm_mul_ps(tmp2, J10)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J01), _mm_mul_ps(tmp2, J11)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J02), _mm_mul_ps(tmp2, J12)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J03), _mm_mul_ps(tmp2, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J04), _mm_mul_ps(tmp2, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J05), _mm_mul_ps(tmp2, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J06), _mm_mul_ps(tmp2, J16)))); pt+=4;


      __m128 tmp3 = _mm_add_ps(_mm_mul_ps(J01, w00), _mm_mul_ps(J11, w01));
      __m128 tmp4 = _mm_add_ps(_mm_mul_ps(J01, w01), _mm_mul_ps(J11, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J01), _mm_mul_ps(tmp4, J11)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J02), _mm_mul_ps(tmp4, J12)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J03), _mm_mul_ps(tmp4, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J04), _mm_mul_ps(tmp4, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J05), _mm_mul_ps(tmp4, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J06), _mm_mul_ps(tmp4, J16)))); pt+=4;


      __m128 tmp5 = _mm_add_ps(_mm_mul_ps(J02, w00), _mm_mul_ps(J12, w01));
      __m128 tmp6 = _mm_add_ps(_mm_mul_ps(J02, w01), _mm_mul_ps(J12, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J02), _mm_mul_ps(tmp6, J12)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J03), _mm_mul_ps(tmp6, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J04), _mm_mul_ps(tmp6, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J05), _mm_mul_ps(tmp6, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J06), _mm_mul_ps(tmp6, J16)))); pt+=4;


      __m128 tmp7 = _mm_add_ps(_mm_mul_ps(J03, w00), _mm_mul_ps(J13, w01));
      __m128 tmp8 = _mm_add_ps(_mm_mul_ps(J03, w01), _mm_mul_ps(J13, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J03), _mm_mul_ps(tmp8, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J04), _mm_mul_ps(tmp8, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J05), _mm_mul_ps(tmp8, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J06), _mm_mul_ps(tmp8, J16)))); pt+=4;


      __m128 tmp9  = _mm_add_ps(_mm_mul_ps(J04, w00), _mm_mul_ps(J14, w01));
      __m128 tmp10 = _mm_add_ps(_mm_mul_ps(J04, w01), _mm_mul_ps(J14, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp9, J04), _mm_mul_ps(tmp10, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp9, J05), _mm_mul_ps(tmp10, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp9, J06), _mm_mul_ps(tmp10, J16)))); pt+=4;


      __m128 tmp11 = _mm_add_ps(_mm_mul_ps(J05, w00), _mm_mul_ps(J15, w01));
      __m128 tmp12 = _mm_add_ps(_mm_mul_ps(J05, w01), _mm_mul_ps(J15, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp11, J05), _mm_mul_ps(tmp12, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp11, J06), _mm_mul_ps(tmp12, J16)))); pt+=4;


      __m128 tmp13 = _mm_add_ps(_mm_mul_ps(J06, w01), _mm_mul_ps(J16, w11));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp13, J06), _mm_mul_ps(tmp13, J16)))); pt+=4;
  ...
  inline void updateSSE_weighted(
          const __m128 J00,const __m128 J01,const __m128 J02,const __m128 J03, const __m128 J04,const __m128 J05,const __m128 J06,
          const __m128 J10,const __m128 J11,const __m128 J12,const __m128 J13, const __m128 J14,const __m128 J15,const __m128 J16,
          const __m128 w,const float a, const float b, const float c)
  {
      __m128 w00 = _mm_set1_ps(a);
      __m128 w01 = _mm_set1_ps(b);
      __m128 w11 = _mm_set1_ps(c);
      ...
      float* pt=SSEData;
      __m128 tmp1 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J00, w00), _mm_mul_ps(J10, w01)));
      __m128 tmp2 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J00, w01), _mm_mul_ps(J10, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J00), _mm_mul_ps(tmp2, J10)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J01), _mm_mul_ps(tmp2, J11)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J02), _mm_mul_ps(tmp2, J12)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp1, J03), _mm_mul_ps(tmp2, J13)))); pt+=4;
      ...
      __m128 tmp3 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J01, w00), _mm_mul_ps(J11, w01)));
      __m128 tmp4 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J01, w01), _mm_mul_ps(J11, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J01), _mm_mul_ps(tmp4, J11)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J02), _mm_mul_ps(tmp4, J12)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J03), _mm_mul_ps(tmp4, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp3, J04), _mm_mul_ps(tmp4, J14)))); pt+=4;
      ...
      __m128 tmp5 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J02, w00), _mm_mul_ps(J12, w01)));
      __m128 tmp6 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J02, w01), _mm_mul_ps(J12, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J02), _mm_mul_ps(tmp6, J12)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J03), _mm_mul_ps(tmp6, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J04), _mm_mul_ps(tmp6, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp5, J05), _mm_mul_ps(tmp6, J15)))); pt+=4;
      ...
      __m128 tmp7 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J03, w00), _mm_mul_ps(J13, w01)));
      __m128 tmp8 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J03, w01), _mm_mul_ps(J13, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J03), _mm_mul_ps(tmp8, J13)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J04), _mm_mul_ps(tmp8, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J05), _mm_mul_ps(tmp8, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp7, J06), _mm_mul_ps(tmp8, J16)))); pt+=4;
      ...
      __m128 tmp9  = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J04, w00), _mm_mul_ps(J14, w01)));
      __m128 tmp10 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J04, w01), _mm_mul_ps(J14, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp9, J04), _mm_mul_ps(tmp10, J14)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp9, J05), _mm_mul_ps(tmp10, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp9, J06), _mm_mul_ps(tmp10, J16)))); pt+=4;

      __m128 tmp11 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J05, w00), _mm_mul_ps(J15, w01)));
      __m128 tmp12 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J05, w01), _mm_mul_ps(J15, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp11, J05), _mm_mul_ps(tmp12, J15)))); pt+=4;
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp11, J06), _mm_mul_ps(tmp12, J16)))); pt+=4;


      __m128 tmp13 = _mm_mul_ps(w, _mm_add_ps(_mm_mul_ps(J06, w01), _mm_mul_ps(J16, w11)));
      _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_add_ps(_mm_mul_ps(tmp13, J06), _mm_mul_ps(tmp13, J16)))); pt+=4;
  ...
  inline void updateSingle(
                  const float* const J0,
                  const float* const J1,
                  const float a, const float b, const float c, int off = 0)
  {
      float* pt=SSEData+off;
      *pt += a*J0[0]*J0[0] + c*J1[0]*J1[0] + b*(J0[0]*J1[0] + J1[0]*J0[0]); pt+=4;
      *pt += a*J0[1]*J0[0] + c*J1[1]*J1[0] + b*(J0[1]*J1[0] + J1[1]*J0[0]); pt+=4;
      *pt += a*J0[2]*J0[0] + c*J1[2]*J1[0] + b*(J0[2]*J1[0] + J1[2]*J0[0]); pt+=4;
      *pt += a*J0[3]*J0[0] + c*J1[3]*J1[0] + b*(J0[3]*J1[0] + J1[3]*J0[0]); pt+=4;
      *pt += a*J0[4]*J0[0] + c*J1[4]*J1[0] + b*(J0[4]*J1[0] + J1[4]*J0[0]); pt+=4;
      *pt += a*J0[5]*J0[0] + c*J1[5]*J1[0] + b*(J0[5]*J1[0] + J1[5]*J0[0]); pt+=4;
      *pt += a*J0[6]*J0[0] + c*J1[6]*J1[0] + b*(J0[6]*J1[0] + J1[6]*J0[0]); pt+=4;


      *pt += a*J0[1]*J0[1] + c*J1[1]*J1[1] + b*(J0[1]*J1[1] + J1[1]*J0[1]); pt+=4;
      *pt += a*J0[2]*J0[1] + c*J1[2]*J1[1] + b*(J0[2]*J1[1] + J1[2]*J0[1]); pt+=4;
      *pt += a*J0[3]*J0[1] + c*J1[3]*J1[1] + b*(J0[3]*J1[1] + J1[3]*J0[1]); pt+=4;
      *pt += a*J0[4]*J0[1] + c*J1[4]*J1[1] + b*(J0[4]*J1[1] + J1[4]*J0[1]); pt+=4;
      *pt += a*J0[5]*J0[1] + c*J1[5]*J1[1] + b*(J0[5]*J1[1] + J1[5]*J0[1]); pt+=4;
      *pt += a*J0[6]*J0[1] + c*J1[6]*J1[1] + b*(J0[6]*J1[1] + J1[6]*J0[1]); pt+=4;

      *pt += a*J0[2]*J0[2] + c*J1[2]*J1[2] + b*(J0[2]*J1[2] + J1[2]*J0[2]); pt+=4;
      *pt += a*J0[3]*J0[2] + c*J1[3]*J1[2] + b*(J0[3]*J1[2] + J1[3]*J0[2]); pt+=4;
      *pt += a*J0[4]*J0[2] + c*J1[4]*J1[2] + b*(J0[4]*J1[2] + J1[4]*J0[2]); pt+=4;
      *pt += a*J0[5]*J0[2] + c*J1[5]*J1[2] + b*(J0[5]*J1[2] + J1[5]*J0[2]); pt+=4;
      *pt += a*J0[6]*J0[2] + c*J1[6]*J1[2] + b*(J0[6]*J1[2] + J1[6]*J0[2]); pt+=4;


      *pt += a*J0[3]*J0[3] + c*J1[3]*J1[3] + b*(J0[3]*J1[3] + J1[3]*J0[3]); pt+=4;
      *pt += a*J0[4]*J0[3] + c*J1[4]*J1[3] + b*(J0[4]*J1[3] + J1[4]*J0[3]); pt+=4;
      *pt += a*J0[5]*J0[3] + c*J1[5]*J1[3] + b*(J0[5]*J1[3] + J1[5]*J0[3]); pt+=4;
      *pt += a*J0[6]*J0[3] + c*J1[6]*J1[3] + b*(J0[6]*J1[3] + J1[6]*J0[3]); pt+=4;


      *pt += a*J0[4]*J0[4] + c*J1[4]*J1[4] + b*(J0[4]*J1[4] + J1[4]*J0[4]); pt+=4;
      *pt += a*J0[5]*J0[4] + c*J1[5]*J1[4] + b*(J0[5]*J1[4] + J1[5]*J0[4]); pt+=4;
      *pt += a*J0[6]*J0[4] + c*J1[6]*J1[4] + b*(J0[6]*J1[4] + J1[6]*J0[4]); pt+=4;


      *pt += a*J0[5]*J0[5] + c*J1[5]*J1[5] + b*(J0[5]*J1[5] + J1[5]*J0[5]); pt+=4;
      *pt += a*J0[6]*J0[5] + c*J1[6]*J1[5] + b*(J0[6]*J1[5] + J1[6]*J0[5]); pt+=4;


      *pt += a*J0[6]*J0[6] + c*J1[6]*J1[6] + b*(J0[6]*J1[6] + J1[6]*J0[6]); pt+=4;
          ...
          const float a, const float b, const float c, int off = 0)
  {
      ...
      float* pt=SSEData+off;
      *pt += weight*(a*J0[0]*J0[0] + c*J1[0]*J1[0] + b*(J0[0]*J1[0] + J1[0]*J0[0])); pt+=4;
      *pt += weight*(a*J0[1]*J0[0] + c*J1[1]*J1[0] + b*(J0[1]*J1[0] + J1[1]*J0[0])); pt+=4;
      *pt += weight*(a*J0[2]*J0[0] + c*J1[2]*J1[0] + b*(J0[2]*J1[0] + J1[2]*J0[0])); pt+=4;
      *pt += weight*(a*J0[3]*J0[0] + c*J1[3]*J1[0] + b*(J0[3]*J1[0] + J1[3]*J0[0])); pt+=4;
      *pt += weight*(a*J0[4]*J0[0] + c*J1[4]*J1[0] + b*(J0[4]*J1[0] + J1[4]*J0[0])); pt+=4;
      *pt += weight*(a*J0[5]*J0[0] + c*J1[5]*J1[0] + b*(J0[5]*J1[0] + J1[5]*J0[0])); pt+=4;
      *pt += weight*(a*J0[6]*J0[0] + c*J1[6]*J1[0] + b*(J0[6]*J1[0] + J1[6]*J0[0])); pt+=4;


      *pt += weight*(a*J0[1]*J0[1] + c*J1[1]*J1[1] + b*(J0[1]*J1[1] + J1[1]*J0[1])); pt+=4;
      *pt += weight*(a*J0[2]*J0[1] + c*J1[2]*J1[1] + b*(J0[2]*J1[1] + J1[2]*J0[1])); pt+=4;
      *pt += weight*(a*J0[3]*J0[1] + c*J1[3]*J1[1] + b*(J0[3]*J1[1] + J1[3]*J0[1])); pt+=4;
      *pt += weight*(a*J0[4]*J0[1] + c*J1[4]*J1[1] + b*(J0[4]*J1[1] + J1[4]*J0[1])); pt+=4;
      *pt += weight*(a*J0[5]*J0[1] + c*J1[5]*J1[1] + b*(J0[5]*J1[1] + J1[5]*J0[1])); pt+=4;
      *pt += weight*(a*J0[6]*J0[1] + c*J1[6]*J1[1] + b*(J0[6]*J1[1] + J1[6]*J0[1])); pt+=4;

      *pt += weight*(a*J0[2]*J0[2] + c*J1[2]*J1[2] + b*(J0[2]*J1[2] + J1[2]*J0[2])); pt+=4;
      *pt += weight*(a*J0[3]*J0[2] + c*J1[3]*J1[2] + b*(J0[3]*J1[2] + J1[3]*J0[2])); pt+=4;
      *pt += weight*(a*J0[4]*J0[2] + c*J1[4]*J1[2] + b*(J0[4]*J1[2] + J1[4]*J0[2])); pt+=4;
      *pt += weight*(a*J0[5]*J0[2] + c*J1[5]*J1[2] + b*(J0[5]*J1[2] + J1[5]*J0[2])); pt+=4;
      *pt += weight*(a*J0[6]*J0[2] + c*J1[6]*J1[2] + b*(J0[6]*J1[2] + J1[6]*J0[2])); pt+=4;


      *pt += weight*(a*J0[3]*J0[3] + c*J1[3]*J1[3] + b*(J0[3]*J1[3] + J1[3]*J0[3])); pt+=4;
      *pt += weight*(a*J0[4]*J0[3] + c*J1[4]*J1[3] + b*(J0[4]*J1[3] + J1[4]*J0[3])); pt+=4;
      *pt += weight*(a*J0[5]*J0[3] + c*J1[5]*J1[3] + b*(J0[5]*J1[3] + J1[5]*J0[3])); pt+=4;
      *pt += weight*(a*J0[6]*J0[3] + c*J1[6]*J1[3] + b*(J0[6]*J1[3] + J1[6]*J0[3])); pt+=4;


      *pt += weight*(a*J0[4]*J0[4] + c*J1[4]*J1[4] + b*(J0[4]*J1[4] + J1[4]*J0[4])); pt+=4;
      *pt += weight*(a*J0[5]*J0[4] + c*J1[5]*J1[4] + b*(J0[5]*J1[4] + J1[5]*J0[4])); pt+=4;
      *pt += weight*(a*J0[6]*J0[4] + c*J1[6]*J1[4] + b*(J0[6]*J1[4] + J1[6]*J0[4])); pt+=4;


      *pt += weight*(a*J0[5]*J0[5] + c*J1[5]*J1[5] + b*(J0[5]*J1[5] + J1[5]*J0[5])); pt+=4;
      *pt += weight*(a*J0[6]*J0[5] + c*J1[6]*J1[5] + b*(J0[6]*J1[5] + J1[6]*J0[5])); pt+=4;


      *pt += weight*(a*J0[6]*J0[6] + c*J1[6]*J1[6] + b*(J0[6]*J1[6] + J1[6]*J0[6])); pt+=4;
